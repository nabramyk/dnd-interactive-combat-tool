...
app.controller('appController', ['$scope', '$rootScope', 'socket', 'globals', function ($scope, $rootScope, socket, globals) {
	...
	$scope.$on('resize', function (_, args) {
		socket.emit('resize', {
			"grid_id": args[0],
	...
	$scope.$on('addElementToServer', function (_, args) {
	...
	$scope.pingPosition = function () {
		$rootScope.$broadcast('ping', [globals.getCursor(), $("#username").val()]);
	};

	$scope.placeElementAction = function () {
		$rootScope.$broadcast('add_element_to_server');
	};
}]);

app.controller('clutterController', ['$scope', '$rootScope', 'globals', function ($scope, $rootScope, globals) {
	var group_grid,
		group_elements,
		group_overlay,
		group_left_ruler,
